...
const MATERIAL_NAME_MAP = {
  'White': 'White', 'Cream': 'Creme', 'Ivory': 'Licht Ivoor', 'WineRed': 'Wijnrood',
  'PineGreen': 'Dennengroen', 'MonumentGreen': 'Monumentengroen', 'BlueSteel': 'Staalblauw',
  'Golden Oak': 'Golden Oak', 'Mahogany': 'Mahonie', 'SilverGrey': 'Zilvergrijs',
  'BasaltGrey': 'Basaltgrijs', 'QuartzGrey': 'Kwartsgrijs', 'Anthracite': 'Antracietgrijs',
  'BlackGrey': 'Zwartgrijs', 'Black': 'Zwart'
};
...
function ModularFrame() {
  ...
  const COLORS = [
    { name: 'White', ral: '9010', hex: '#f4f4f4' }, { name: 'Cream', ral: '9001', hex: '#fdf4d3' },
    { name: 'Ivory', ral: '1015', hex: '#eae3c6' }, { name: 'WineRed', ral: '3005', hex: '#5e1b22' },
    { name: 'PineGreen', ral: '6009', hex: '#1c3c1b' }, { name: 'MonumentGreen', ral: '6005', hex: '#2f4538' },
    { name: 'BlueSteel', ral: '5011', hex: '#232c3f' }, { name: 'Golden Oak', ral: '8003', hex: '#a75b1f' },
    { name: 'Mahogany', ral: '8016', hex: '#4c2f27' }, { name: 'SilverGrey', ral: '7001', hex: '#c0c0c0' },
    { name: 'BasaltGrey', ral: '7012', hex: '#4e5754' }, { name: 'QuartzGrey', ral: '7039', hex: '#6c6860' },
    { name: 'Anthracite', ral: '7016', hex: '#373f43' }, { name: 'BlackGrey', ral: '7021', hex: '#2e3234' },
  ...
  useEffect(() => {
    if (controller && modelReady && MATERIAL_NAME_MAP[modularColor.name]) {
      controller.setMaterialForZone('outside', MATERIAL_NAME_MAP[modularColor.name]);
    }
  }, [controller, modelReady, modularColor]);

  useEffect(() => {
    if (controller && modelReady) controller.setWallVisible(wallEnabled);
  ...
  const handleSubmit = async () => {
    const payload = {
      model, width, height, frameColor: frameColor.name,
      insideColor: insideColor.name, modularColor: modularColor.name,
      horizontalEnabled, verticalEnabled, ModuleHorizontal, ModuleVertical,
    };
          ...
          <ul>
            ...
            <li className="modular-block">
              <div className="toggle-wrapper">
                ...
                <button className={`mode-toggle ${horizontalEnabled ? 'enabled' : ''}`} onClick={() => setHorizontalEnabled(!horizontalEnabled)}>{horizontalEnabled ? '✓' : ''}</button>
              </div>
            </li>
            <li className="modular-block">
              <div className="toggle-wrapper">
                <SliderControl label="Vertical Modulization" min={250} max={1200} value={ModuleVertical} onChange={setVertical} />
                <button className={`mode-toggle ${verticalEnabled ? 'enabled' : ''}`} onClick={() => setVerticalEnabled(!verticalEnabled)}>{verticalEnabled ? '✓' : ''}</button>
              </div>
            </li>
            <li><ColorSelectorGroup title="Frame Color" colors={COLORS} selected={frameColor} onSelect={setFrameColor} /></li>
            <li><ColorSelectorGroup title="Inside Frame Color" colors={COLORS} selected={insideColor} onSelect={setInsideColor} /></li>
            <li><ColorSelectorGroup title="Modular Frame Color" colors={COLORS} selected={modularColor} onSelect={setModularColor} /></li>

            <li className="toggle-wrapper">
              <span>Show Wall</span>
              <button
                className={`mode-toggle ${wallEnabled ? 'enabled' : ''}`}
                onClick={() => wallToggleEnabled && setWallEnabled(prev => !prev)}
